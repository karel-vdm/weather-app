...
class TurnOnLocationView @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : FrameLayout(context, attrs, defStyleAttr) {

    private var turnOnLocationButton: MaterialButton? = null

    var onTurnOnLocationClickListener: () -> Unit = {}

    init {
        View.inflate(context, R.layout.layout_turn_on_location, this)
        turnOnLocationButton = findViewById(R.id.turn_on_location_button)
        turnOnLocationButton?.setOnClickListener {
            onTurnOnLocationClickListener.invoke()
        }
    }
